      ...
      else if (cmd == ac)
      {
        ...
        if (argVal_cmd == "on") {
          ...
          if (send_msg_slave(msg_to_slave, sizeof(msg_to_slave))) {
            Serial.println("The slave got the message. AC is ON");        // note - we cannot be sure if AC is on because maybe we got ACK for a partial message
          }
          else {
          ...
          if (send_msg_slave(msg_to_slave, sizeof(msg_to_slave))) {
            Serial.println("The slave got the message. AC is OFF");        // note - we cannot be sure if AC is on because maybe we got ACK for a partial message
          }
          else {
            Serial.println("Something went wrong. Try to send again the message");
          } 
...
int send_msg_slave(char* msg_to_slave, int sz) {
  ...
  while (Wire.available()) {                    // slave may send less than requested
    ...
    if (!strcmp(c, msg_ack_from_slave[i])) {
      i++;    
    }
    else {
      Serial.println();
